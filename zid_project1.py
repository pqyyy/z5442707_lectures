...
def line_to_dict(line):
    ...
    for column in COLUMNS:
        ...
        end_i = start_i + col
        dict[column] = line[start_i:end_i]
        start_i = end_i
    return dict
    ...
    if tickers_lst is not None:
        if tickers_lst == []:
            ...
            for ticker in tickers_lst:
                if ticker not in list(tic_exchange_dic.keys()):
                    raise Exception(f"{ticker} does not correspond to a key tic_exchange_dic")
        ...
        if col_lst == []:
            ...
            for col in col_lst:
                if col not in COLUMNS:
                    raise Exception(f"{col} does not found in 'COL""" zid_project1.py
...
def verify_cols(col_lst=None):
    ...
    if col_lst is not None:
        if col_lst == []:
            ...
            for col in col_lst:
                if col not in COLUMNS:
                    raise Exception(f"{col} does not found in 'COLUMNS'")
